...
// GeekjobJSParser парсер для Geekjob.ru с поддержкой JavaScript
type GeekjobJSParser struct {
	browser         *rod.Browser
...
// NewGeekjobJSParser создает новый экземпляр GeekjobJSParser
func NewGeekjobJSParser(databaseService string, timeout, delay time.Duration) *GeekjobJSParser {
	// Запускаем браузер
	browser := rod.New().MustConnect()

	return &GeekjobJSParser{
		browser:         browser,
...
func (p *GeekjobJSParser) ParseAllSources(query string, pages int) ([]models.Vacancy, error) {
	logrus.Infof("Starting Geekjob.ru JS parsing: query='%s', pages=%d", query, pages)
	...
	logrus.Infof("Geekjob.ru JS parsing completed. Total: %d vacancies", len(allVacancies))
	return allVacancies, nil
}

func (p *GeekjobJSParser) parseVacancyListPage(query string, page int) ([]models.Vacancy, error) {
	url := fmt.Sprintf("https://geekjob.ru/vacancies?q=%s&page=%d", url.QueryEscape(query), page)
	logrus.Infof("Parsing Geekjob.ru JS page %d: %s", page, url)
...
func (p *GeekjobJSParser) extractVacancyID(url string) string {
...
func (p *GeekjobJSParser) isRelevantVacancy(title, description string) bool {
