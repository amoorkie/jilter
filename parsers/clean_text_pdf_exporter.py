#!/usr/bin/env python3
"""
Чистый экспорт вакансий в PDF из текста (без HTML/JS)
"""
...
class CleanTextPDFExporter:
    """Чистый экспорт вакансий в PDF из текста"""
    ...
    def create_clean_pdf(self, vacancies: List[Dict[str, Any]], output_path: str) -> bool:
        """Создание чистого PDF из текста"""
        try:
            pdf = FPDF()
            pdf.add_page()
            ...
            for i, vacancy in enumerate(vacancies, 1):
                ...
                pdf.multi_cell(0, 6, clean_title.encode('latin-1', 'replace').decode('latin-1'))
                pdf.ln(2)
    ...
    def export_vacancies_to_pdf(self, output_path: str = None) -> bool:
        ...
        if not output_path:
            ...
            output_path = f"vacancies_clean_text_{timestamp}.pdf"
        
        print("Чистый экспорт вакансий в PDF (из текста)")
        print("="*50)
        ...
        # Создаем PDF
        success = self.create_clean_pdf(vacancies, output_path)
...
def main():
    ...
    # Создаем экспортер
    exporter = CleanTextPDFExporter(db_path)
