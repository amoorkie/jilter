#!/usr/bin/env python3
"""
Экспорт вакансий в PDF точно как в админке - только чистый текст
"""
...
class AdminLikePDFExporter:
    """Экспорт вакансий в PDF как в админке"""
    ...
    def extract_clean_text(self, html_text: str) -> str:
        """Извлечение чистого текста как в админке"""
        if not html_text:
            return ""
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in [
                ...
                'liveinternet', 'kraken', 'counter', 'project:',
                'st.top100', 'd.createElement', 'https://', 'http://'
            ]):
                continue
        ...
        try:
            ...
            for i, vacancy in enumerate(vacancies, 1):
                ...
                if vacancy['full_description']:
                    # Извлекаем чистый текст как в админке
    ...
    def export_vacancies_to_pdf(self, output_path: str = None) -> bool:
        ...
        if not output_path:
            ...
            output_path = f"vacancies_admin_like_{timestamp}.pdf"
        
        print("Экспорт вакансий в PDF (как в админке)")
        print("="*50)
...
def main():
    ...
    # Создаем экспортер
    exporter = AdminLikePDFExporter(db_path)
