#!/usr/bin/env python3
"""
Простой экспорт вакансий в PDF с минимальными зависимостями
"""
...
class SimplePDFExporter:
    """Простой экспорт вакансий в PDF"""
    ...
    def clean_text(self, text: str) -> str:
        """Очистка текста как в админке - только чистый текст"""
        if not text:
            return ""
        ...
        # Убираем JavaScript код и скрипты
        ...
        # Убираем JavaScript код в тексте
        clean_text = re.sub(r'var\s+\w+\s*=.*?;', '', clean_text, flags=re.DOTALL)
        clean_text = re.sub(r'function\s+\w+\(.*?\).*?}', '', clean_text, flags=re.DOTALL)
        clean_text = re.sub(r'window\.\w+\s*=.*?;', '', clean_text, flags=re.DOTALL)
        clean_text = re.sub(r'document\.\w+.*?;', '', clean_text, flags=re.DOTALL)
        
        # Убираем URL и ссылки на скрипты
        clean_text = re.sub(r'https?://[^\s]+', '', clean_text)
        clean_text = re.sub(r'\.js[^\w]', '', clean_text)
        clean_text = re.sub(r'\.css[^\w]', '', clean_text)
        ...
        try:
            ...
            for i, vacancy in enumerate(vacancies, 1):
                ...
                if vacancy['full_description']:
                    # Очищаем описание
                    clean_description = self.clean_text(vacancy['full_description'])
    ...
    def export_vacancies_to_pdf(self, output_path: str = None) -> bool:
        ...
        if not output_path:
            ...
            output_path = f"vacancies_simple_{timestamp}.pdf"
        
        print("Простой экспорт вакансий в PDF")
        print("="*50)
...
def main():
    ...
    # Создаем экспортер
    exporter = SimplePDFExporter(db_path)
