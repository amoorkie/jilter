#!/usr/bin/env python3
"""
Чистый экспорт вакансий в PDF без JavaScript кода
"""
...
class CleanPDFExporter:
    """Чистый экспорт вакансий в PDF без кода"""
    ...
    def aggressive_clean_text(self, text: str) -> str:
        """Агрессивная очистка текста - убираем весь код"""
        if not text:
            return ""
        ...
        # Убираем JavaScript код (более агрессивно)
        patterns_to_remove = [
            r'<script[^>]*>.*?</script>',
            r'<style[^>]*>.*?</style>',
            r'<noscript[^>]*>.*?</noscript>',
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in [
                ...
                'top100', 'telegram', 'vkontakte', 'mail.ru'
            ]):
                continue
    ...
    def create_pdf(self, vacancies: List[Dict[str, Any]], output_path: str) -> bool:
        ...
        try:
            ...
            for i, vacancy in enumerate(vacancies, 1):
                ...
                # Очищаем заголовок
                clean_title = self.aggressive_clean_text(vacancy['title'])
                story.append(Paragraph(clean_title, company_style))
                
                # Компания
                clean_company = self.aggressive_clean_text(vacancy['company'])
                ...
                if vacancy['full_description']:
                    # Агрессивно очищаем описание
                    clean_description = self.aggressive_clean_text(vacancy['full_description'])
    ...
    def export_vacancies_to_pdf(self, output_path: str = None) -> bool:
        ...
        if not output_path:
            ...
            output_path = f"vacancies_clean_{timestamp}.pdf"
        
        print("Чистый экспорт вакансий в PDF (без кода)")
        print("="*50)
...
def main():
    ...
    # Создаем экспортер
    exporter = CleanPDFExporter(db_path)
