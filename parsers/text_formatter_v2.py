"""
Вариант 2: Извлечение только абзацев и списков без заголовков
"""
...
def extract_content_without_headers(element) -> str:
    """
    Извлекает контент без заголовков (h1-h6)
    - Только абзацы (p, div)
    - Только списки (ul, ol, li)
    - Без заголовков
    """
    if not element:
        return ''
    ...
    def process_element(elem, level=0):
        ...
        if tag_name in ['ul', 'ol']:
            ...
            # Абзацы и блоки (но не заголовки)
            for child in elem.children:
                process_element(child, level + 1)
            result.append('\n\n')
...
def extract_paragraphs_and_lists_only(soup, selectors):
    """
    Вариант 2: Извлекает только абзацы и списки без заголовков
    """
    content = ""
    ...
    for selector in selectors:
        ...
        if element:
            content = extract_content_without_headers(element)
            if content:
                break
...
def extract_formatted_text(element):
    ...
    return extract_content_without_headers(element)


def extract_structured_sections(full_description: str) -> Dict[str, str]:
    """
    Возвращает пустые секции, так как в варианте 2 мы не разбиваем на блоки
    """
    return {
        'requirements': '',
        'tasks': '',
