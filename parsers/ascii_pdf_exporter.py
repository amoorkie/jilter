#!/usr/bin/env python3
"""
ASCII PDF экспорт вакансий (только английские символы)
"""
...
class ASCIIPDFExporter:
    """ASCII PDF экспорт (только английские символы)"""
    ...
    def create_ascii_pdf(self, vacancies: List[Dict[str, Any]], output_path: str) -> bool:
        """Создание ASCII PDF"""
        try:
            pdf = FPDF()
            pdf.add_page()
            ...
            pdf.cell(0, 10, "VACANCY EXPORT FOR CHATGPT PROCESSING", 0, 1, "C")
            pdf.ln(10)
            
            # Метаинформация
            pdf.set_font("Arial", "", 10)
            export_date = datetime.now().strftime("%d.%m.%Y %H:%M")
            pdf.cell(0, 5, f"Export date: {export_date}", 0, 1)
            pdf.cell(0, 5, f"Number of vacancies: {len(vacancies)}", 0, 1)
            pdf.ln(10)
            ...
            for i, vacancy in enumerate(vacancies, 1):
                ...
                pdf.cell(0, 8, f"VACANCY #{vacancy['id']}", 0, 1)
                
                # Заголовок
                pdf.set_font("Arial", "B", 12)
                clean_title = self.extract_clean_text(vacancy['title'])
                translit_title = self.transliterate_text(clean_title)
                pdf.multi_cell(0, 6, translit_title)
                pdf.ln(2)
                
                # Компания
                pdf.set_font("Arial", "", 10)
                clean_company = self.extract_clean_text(vacancy['company'])
                translit_company = self.transliterate_text(clean_company)
                pdf.cell(0, 5, f"Company: {translit_company}", 0, 1)
                
                # Источник и дата
                pdf.cell(0, 5, f"Source: {vacancy['source']}", 0, 1)
                if vacancy['published_at']:
                    published_date = vacancy['published_at'][:10] if len(vacancy['published_at']) > 10 else vacancy['published_at']
                    pdf.cell(0, 5, f"Date: {published_date}", 0, 1)
                ...
                if vacancy['full_description']:
                    ...
                    translit_description = self.transliterate_text(clean_description)
                    
                    if translit_description.strip():
                        # Разбиваем текст на абзацы
                        paragraphs = translit_description.split('\n')
                        for paragraph in paragraphs:
                            if paragraph.strip():
                                pdf.multi_cell(0, 5, paragraph)
                                pdf.ln(2)
                    else:
                        pdf.cell(0, 5, "[Text not found after cleaning]", 0, 1)
    ...
    def export_vacancies_to_pdf(self, output_path: str = None) -> bool:
        ...
        if not output_path:
            ...
            output_path = f"vacancies_ascii_{timestamp}.pdf"
        
        print("ASCII экспорт вакансий в PDF")
        print("="*50)
        ...
        # Создаем PDF
        success = self.create_ascii_pdf(vacancies, output_path)
...
def main():
    ...
    # Создаем экспортер
    exporter = ASCIIPDFExporter(db_path)
