#!/usr/bin/env python3
"""
Экспорт вакансий в PDF используя только чистые поля без HTML
"""
...
class CleanFieldsPDFExporter:
    """Экспорт вакансий в PDF используя только чистые поля"""
    ...
    def get_vacancies_for_export(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, title, company, description, requirements, tasks, conditions, benefits, source, published_at
                FROM vacancies 
                WHERE (description IS NOT NULL AND description != '') 
                OR (requirements IS NOT NULL AND requirements != '')
                OR (tasks IS NOT NULL AND tasks != '')
                OR (conditions IS NOT NULL AND conditions != '')
                OR (benefits IS NOT NULL AND benefits != '')
                ORDER BY id
            """)
    ...
    def build_vacancy_description(self, vacancy: Dict[str, Any]) -> str:
        """Сборка описания вакансии из чистых полей"""
        parts = []
    ...
    def create_pdf(self, vacancies: List[Dict[str, Any]], output_path: str) -> bool:
        ...
        try:
            ...
            for i, vacancy in enumerate(vacancies, 1):
                ...
                # Описание из чистых полей
                description = self.build_vacancy_description(vacancy)
                if description:
    ...
    def export_vacancies_to_pdf(self, output_path: str = None) -> bool:
        ...
        if not output_path:
            ...
            output_path = f"vacancies_clean_fields_{timestamp}.pdf"
        
        print("Экспорт вакансий в PDF (чистые поля)")
        print("="*50)
...
def main():
    ...
    # Создаем экспортер
    exporter = CleanFieldsPDFExporter(db_path)
